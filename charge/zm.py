...
async def sa(Client, message):
    try:
        ...
        if (str(message.chat.id) + "\n" not in verified_gps and message.chat.type != "private"):
            await message.reply_text(text="""<b>This Group Is Not Verified. Talk With <code>@Sarcehkr</code> And Ask For Verification.</b>""",reply_to_message_id=message.message_id)
        else:
            text = f"""
...
<b>○</b> BOT BY: <b>@SarceMosra</b>"""
                                        ...
                                        try:
                                            ...
                                            if response is None:
                                                await msg.edit_text("PROXY DEAD PLEASE REPORT TO OWNER @Sarcehkr")
                                            else:
...
<b>○</b> BOT BY: <b>@SarceMosra</b>"""
        ...
        await msg.edit_text("PROXY DEAD PLEASE REPORT TO OWNER <code>@Sarcehkr</code>")
